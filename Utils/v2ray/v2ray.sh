...
#BASE_SOURCE_PATH="https://github.com/lacasitamx/multi_v2ray"
...
removeV2Ray() {
 ...
 bash <(curl -L -s https://github.com/lacasitamx/multi_v2ray/raw/master/go.sh) --remove >/dev/null 2>&1
    rm -rf /etc/v2ray >/dev/null 2>&1
    rm -rf /var/log/v2ray >/dev/null 2>&1

    #卸载Xray脚本
 #   bash <(curl -L -s https://multi.netlify.app/go.sh) --remove -x >/dev/null 2>&1
 bash <(curl -L -s https://github.com/lacasitamx/multi_v2ray/raw/master/go.sh) --remove -x >/dev/null 2>&1
...
EOF
    ...
    #安装V2ray主程序
    [[ ${INSTALL_WAY} == 0 ]] && bash <(curl -L -s https://www.dropbox.com/s/p8mowsmm7l1n9cu/go.sh)
}
...
installFinish(){
    ...
    jq '.inbounds[].streamSettings += {"network":"ws","wsSettings":{"path": "/NEAR365/","headers": {"Host": "ejemplo.com"}}}' < /etc/v2ray/tmp.json >> /etc/v2ray/config.json
